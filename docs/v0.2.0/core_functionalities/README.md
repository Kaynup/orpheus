...
| **09** | [**Pipeline Modularity & Interfaces**](./09_pipeline_modularity_and_interfaces.md) | **NFR1** | Abstract base pipeline hierarchy (`BasePipeline`, `BaseIngestionPipeline`, `BaseInferencePipeline`, `BaseRAGPipeline`), sub-pipeline extraction (`DocumentIngestionPipeline`, `QueryInferencePipeline`), DI factory functions, and lightweight facade orchestrator. |
